...
norm_test_out <-
  ...
  summarize(
    `pGauss (KS)` = ksnormal(body_mass, lillie = FALSE),
    `pGauss (Shapiro)` = shapiro.test(body_mass) |>
      pluck("p.value"),
    .groups = "drop"
  )
...
normaltable |>
  ...
  #    bg = 'deeppink') |>
  bg(~ str_detect(Chinstrap, "\\*"),
    j = 3,
    bg = "deeppink"
  ) |>
  bg(~ str_detect(Gentoo, "\\*"),
    j = 4,
    bg = "deeppink"
  ) |>
  color(~ !Adelie < .05,
...
normality_test <-
  ...
  pivot_longer(
    cols = where(is.character), # contains("_")
    names_to = "Measure",
